...
function Accordion() {
    ...
    const getExhibits = async (exhibit) => {
      ...
      if(exhibit) { 
        if (!isNaN(exhibit)) { // is numeric
          ...
          response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/exhibits/`);
      }
    ...
    const getSelectedAnimals = async function(exhibitName) {
      let response;
      response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/animals/exhibits/${exhibitName}`)
